...
class StudentCheckStatusPage extends StatefulWidget {
  final int userId;
  const StudentCheckStatusPage({super.key, required this.userId});
...
class _StudentCheckStatusPageState extends State<StudentCheckStatusPage> {
  ...
  Future<void> loadBooking() async {
    ...
    // only show today's booking that is Pending or Approved
    ...
    final latest = bookings.firstWhere(
        ...
        orElse: () => {});
    setState(() {
      currentBooking = latest.isNotEmpty ? latest : null;
      loading = false;
    });
  }
